...
export default function RiskMap({ riskData, userLocation }) {
  ...
  const [showBM21, setShowBM21] = useState(true)
  const [showPHL03, setShowPHL03] = useState(true)
  ...
  return (
    <div className="h-full relative">
      ...
      <div className="absolute top-4 right-4 z-[1000] bg-slate-900/90 backdrop-blur-sm rounded-lg p-3 space-y-2">
        <p className="text-xs text-slate-400 font-semibold mb-2">Toggle Layers</p>
        ...
        <label className="flex items-center gap-2 text-sm text-orange-400 cursor-pointer">
          <input
            type="checkbox"
            checked={showBM21}
            onChange={(e) => setShowBM21(e.target.checked)}
            className="rounded"
          />
          BM-21 (52km)
        </label>
        <label className="flex items-center gap-2 text-sm text-yellow-400 cursor-pointer">
          <input
            type="checkbox"
            checked={showPHL03}
            onChange={(e) => setShowPHL03(e.target.checked)}
            className="rounded"
          />
          PHL-03 (130km)
        </label>
      ...
      {/* Map Legend */}
      <div className="absolute bottom-4 left-4 z-[1000] bg-slate-900/90 backdrop-blur-sm rounded-lg p-3 text-xs">
        <p className="text-slate-400 font-semibold mb-2">Risk Zones</p>
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-red-600 rounded-full"></div>
            <span className="text-white">High Danger (&lt;20km)</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white">Safe (&gt;130km)</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {/* Province Polygons */}
        {showProvinces && borderProvinces.map((province) => (
          <Polygon
            key={province.name}
            ...
            <Marker position={[userLocation.lat, userLocation.lon]} icon={userIcon}>
              <Popup>
                <div className="text-center">
                  ...
                  {riskData && (
                    <>
                      <span className="font-semibold">{riskData.distance_km} km</span> to border<br />
        ...
        >
          <Popup>Thai-Cambodia Border</Popup>
        </Polygon>
      </MapContainer>
    </div>
  )
